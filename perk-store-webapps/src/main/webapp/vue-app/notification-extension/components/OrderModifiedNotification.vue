...
export default {
  ...
  computed: {
    ...
    modifierId() {
      return this.notification?.parameters?.MODIFIER_ID;
    },
    senderId() {
      return this.notification?.parameters?.SENDER_ID;
    },
    isSender() {
      return this.senderId === eXo.env.portal.userIdentityId;
    },
    orderStatus() {
      return this.notification?.parameters?.ORDER_STATUS?.toLowerCase();
    },
    orderUrl() {
    ...
    message() {
      return this.modifierId && this.$t('Notification.perkstore.order.modified', {
        0: `<a class="user-name font-weight-bold">${this.profile?.fullname}</a>`,
      }) || this.$t(`Notification.perkstore.order.modified.short.${this.orderStatus}.system${this.isSender && '.you' || ''}`, {
        0: `<a class="user-name font-weight-bold">${this.profile?.fullname}</a>`,
      });
    },
  },
